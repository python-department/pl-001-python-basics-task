...
def decode_timestamp_ms(snowflake_id: int, epoch_ms: int = EPOCH_MS_DEFAULT) -> int:
    ...
    Returns:
        ...
        generated.
    """
    stored_ms = (snowflake_id >> TIMESTAMP_SHIFT) & TIMESTAMP_MS_MAX
...
def decode_sequence_id(snowflake_id: int) -> int:
    ...
    """
    return (snowflake_id >> 0) & SEQUENCE_ID_MAX
...
def generate_snowflake_id(
    ...
    if elapsed_ms > TIMESTAMP_MS_MAX:
        print(
            f"timestamp overflows: elapsed {elapsed_ms} ms exceeds \n\
              TIMESTAMP_MS_MAX ({TIMESTAMP_MS_MAX})"
        )
        return None
